...
void Astros::render(){
	//this->Pc = Vetor3f(this->distOrb*cos(this->angOrb *this->tempo ),this->distOrb*sin(this->angOrb *this->tempo),0);
...
Planeta::Planeta(int aQtdLuas,int aOrbita,Astros* aAstroCentral): Astros(0.15,0.25,aQtdLuas,aOrbita,aAstroCentral){
	...
	//this->Pc = Vetor3f(this->distOrb*sin(this->angOrb),this->distOrb*cos(this->angOrb),0);
	this->Pc = Vetor3f(this->angOrb,0,0);
	this->material.cor = Cor3f(1.0,0.0,0.0);
	this->material.ka = 0.1;
	this->material.kd = 0.8;
	this->material.ks = 0.3;
	this->material.n  = 200;
...
Strela::Strela(int aQtdPlanetas) : Astros(0.70,1.20,aQtdPlanetas,0,NULL){
	this->r = this->raio;
	//this->Pc = Vetor3f(0,-15,0);
	this->Pc = Vetor3f(0,0,0);
	this->material.cor = Cor3f(1.0,1.0,0.0);
	this->material.ka = 1.0;
	this->material.kd = 0.1;
	this->material.ks = 0.1;
	this->material.n  = 0;
...
void Sistema::getAstros(IObjeto** objetos){

	
	objetos[0] = this->estrela;

	for(int i = 0; i < this->estrela->qtdSatelites; i++)
	{
		if(i % 2 == 0)
			this->estrela->satelites[i]->Pc.z = -5.0;
		objetos[i+1] = this->estrela->satelites[i];
	}

	


	// objetos[1] = this->estrela->satelites[1];
